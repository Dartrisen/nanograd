...
class TopologyEvolver:
    ...
    def evolve_memory(self, memory: TNMemory) -> bool:
        ...
        if ideal_rank != curr_bond:
            ...
            # Adapt output predictive tracking head and embedding size to support the new state size.
            ...
            new_embed_data = np.random.randn(vocab_size, ideal_rank) * 0.05
            if hasattr(memory, "W_embed"):
                new_embed_data[:vocab_size, :copy_bound] = memory.W_embed.data[:, :copy_bound]
